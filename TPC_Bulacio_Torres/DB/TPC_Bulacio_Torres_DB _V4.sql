...
CREATE TABLE StopCode(
	...
	StopCodeState bit not null default (1)
)
GO 
...
END
GO
CREATE TRIGGER TR_DELETE_STOPCODE ON StopCode
INSTEAD OF DELETE
AS
BEGIN
	UPDATE StopCode SET StopCodeState = 0 WHERE IDStopCode IN (SELECT IDStopCode FROM deleted)
END
GO
